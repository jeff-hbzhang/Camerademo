...
document.addEventListener("appMobi.device.ready",appMobiReady,false);
...
if( typeof(AppMobi) == "undefined" ) {
	var amscr = document.createElement("script");
	amscr.type = "text/javascript";
	amscr.onload = appMobiLoaded;
	amscr.src = "http://localhost:58888/_appMobi/appmobi.js";
	document.getElementsByTagName('head')[0].appendChild(amscr);
} else if(typeof(AppMobi) == "object" && typeof(AppMobi.device) == "undefined" ) {
...
function appMobiLoaded()
{
	amloaded = true;
	loadCordova();
}

function appMobiReady()
{
	amready = true;
	loadCordova();
}
...
{
	if( amloaded == true && amready == true )
	{
		AppMobi.exec("AppMobiDevice.registerLibrary", "CordovaModule");
	}
}

function orientationChange(evt)
{
	// forward to the cordova event system
	window.__defineGetter__('orientation',function(){ return AppMobi.device.orientation; });
...
function CordovaLoaded()
{
	...
	window.channel.onNativeReady.fire();
}
...
function iOSExec() {
    ...
    if (!isInContextOfEvalJs && commandQueue.length == 1) {
	    ...
	    //xhr.js compatibility
	    execXhr = execXhr || (typeof nativeXMLHttpRequest != "undefined" ?
		new nativeXMLHttpRequest() : new XMLHttpRequest());
...
window.channel = require('cordova/channel');
...
// do not fire onNativeReady from here - it gets fired after the module is loaded
